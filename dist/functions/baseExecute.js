"use strict";
const { ActionRowBuilder, TextInputBuilder, ModalBuilder, TextInputStyle, EmbedBuilder, ModalSubmitInteraction, } = require("discord.js");
...
const baseModal = async (customId, question1, question2, placeholder1, placeholder2, interaction) => {
    ...
    const aboutName = new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setLabel("Ваше имя, возраст?")
        .setCustomId("Ваше имя, возраст?")
        .setRequired(true)
        .setPlaceholder("Ален 18 лет")
        .setStyle(TextInputStyle.Short));
    const aboutExperience = new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setLabel("Был ли опыт на этой должности")
        .setCustomId("Был ли опыт на этой должности")
        .setRequired(true)
        .setPlaceholder("2 года на проекте..")
        .setStyle(TextInputStyle.Short));
    const whyYou = new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId("Почему именно вы?")
        .setLabel("Почему именно вы?")
        .setPlaceholder("Потому что я Ален...")
        .setRequired(true)
        .setStyle(TextInputStyle.Paragraph));
    const question1_ = new ActionRowBuilder().addComponents(new TextInputBuilder()
        .setCustomId(question1)
        .setLabel(question1)
        .setStyle(TextInputStyle.Paragraph)
        .setRequired(true)
        .setPlaceholder(placeholder1));
    const question2_ = new ActionRowBuilder().addComponents(new TextInputBuilder()
...
const baseCallback = async (interaction) => {
    const embed = new EmbedBuilder().setTitle(`Вакансия от ${interaction.user.username} на должность ${interaction.customId}`);
