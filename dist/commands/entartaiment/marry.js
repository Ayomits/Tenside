...
const { CommandInteraction, EmbedBuilder, ButtonBuilder, ButtonStyle, ActionRowBuilder, ComponentType, } = require("discord.js");
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        const requestEmbed = new EmbedBuilder()
            .setTitle("Система браков")
            .setDescription(`Уважаемый пользователь <@${target.id}>, хотите ли вы заключить брак с пользователем <@${author.id}>?`)
                ...
                if (buttonInteraction.user.id === target.id) {
                    ...
                    if (updated1 && marry) {
                        const acceptEmbed = new EmbedBuilder()
                            .setTitle("Система браков")
                            .setDescription(`**Я** - как посланник моего разработчика, объявляю брак между <@${author.id}> и <@${target.id}> успешным. Пусть в вашей дальнейшей совместной жизни будет много счастья и любви!`)
