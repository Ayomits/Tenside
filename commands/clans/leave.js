  ...
  async execute(interaction) {
    ...
    if (result !== null) {
      ...
      if (result.clanDeputy.includes(interaction.user.id)) {
        await clanModel.updateOne({guild_id: interaction.user.id, clanName: result.clanName}, {$pull: {clanMembers: interaction.user.id, clanDeputy: interaction.user.id}})
        await interaction.member.roles.remove(result.clanRole)
        return await interaction.reply({content: "клан успешно покинут"})
      }
      await clanModel.updateOne({guild_id: interaction.user.id, clanName: result.clanName}, {$pull: {clanMembers: interaction.user.id}})
      await interaction.member.roles.remove(result.clanRole)
      return await interaction.reply({content: "клан успешно покинут"})
    } else {
